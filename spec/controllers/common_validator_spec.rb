  ...
  describe '#valid' do
    ...
    context 'validation 2 にはじかれる（数字）' do
      ...
      it '小数' do
        ...
        expect(valid{@post}).to eq "1番目のカード指定文字が不正です。(S1.1) ,半角英字大文字のスート（S,H,D,C）と数字（1〜13）の組み合わせでカードを指定してください。"
      end
      it '演算' do
        @post = "S2*3 S10 S11 S12 S13"
        expect(valid{@post}).to eq "1番目のカード指定文字が不正です。(S2*3) ,半角英字大文字のスート（S,H,D,C）と数字（1〜13）の組み合わせでカードを指定してください。"
      end
    end
